...
class MyTable:
    ...
    def clean_column_company(self):
        ...
        #Fix unique bad rows in this column.
        company_column_clean[90] = "Jim's Formal Wear, LLC"
    ...
    def clean_column_effective_date(self):
        ...
        for row in effective_date_column_raw:
            clean_row_list = self.get_data_between_tags(str(row))
            clean_row = clean_row_list[0]
            effective_date_column_clean.append(clean_row)
    ...
    def clean_column_worker_count(self):
        ...
        #Fix unique bad rows in this column
        worker_count_column_clean[7] = "97 in MI, 0 in IN"
    ...
    def get_row_from_string(self, string_to_find, column):
        column_to_check = eval("self." + column)
        indexes = [i for i, x in enumerate(column_to_check) if string_to_find in x]
        for row in indexes:
            print(str(row) + ": " + column_to_check[row])
